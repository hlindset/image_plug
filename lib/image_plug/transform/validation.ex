defmodule ImagePlug.Transform.Validation do
  ...
  def keys!(attrs, allowed_keys, label) do
    ...
    if unknown_keys != [] do
      ...
      raise ArgumentError, "unknown #{label} option(s): #{keys}"
    end
  end
  ...
  def positive_dimension_pair!(label, width, height) do
    positive_dimension_or_auto!(label, :width, width)
    positive_dimension_or_auto!(label, :height, height)

    if width == :auto and height == :auto do
      raise ArgumentError, "invalid #{label} dimensions: width and height cannot both be :auto"
    end
  end
  ...
  def dimension_rule!(label, field, %DimensionRule{} = rule, modes) do
    case DimensionRule.validate(rule, modes: modes) do
      ...
      {:error, {invalid_field, value}} ->
        raise ArgumentError, "invalid #{label} #{field} #{invalid_field}: #{inspect(value)}"
    end
  end
  ...
  def invalid!(label, field, value) do
    raise ArgumentError, "invalid #{label} #{field}: #{inspect(value)}"
  end

  def invalid_options!(label, attrs) do
    raise ArgumentError, "invalid #{label} options: #{inspect(attrs)}"
  end
end
