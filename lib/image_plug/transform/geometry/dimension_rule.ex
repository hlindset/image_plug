defmodule ImagePlug.Transform.Geometry.DimensionRule do
  ...
  @spec resolve(t(), keyword()) :: {:ok, resolved_dimensions()} | {:error, term()}
  def resolve(%__MODULE__{} = rule, opts) when is_list(opts) do
  ...
  defp normalize_rule(%__MODULE__{} = rule) do
  ...
  defp resolve_base_dimensions(%__MODULE__{width: :auto, height: :auto} = rule, source) do
  ...
  defp resolve_base_dimensions(%__MODULE__{mode: :fit} = rule, source) do
    rule
  ...
  defp resolve_base_dimensions(%__MODULE__{mode: :fill} = rule, source) do
    rule
    |> requested_box(source)
    |> apply_zoom(rule)
    |> ok()
  ...
  defp resolve_base_dimensions(%__MODULE__{mode: :fill_down} = rule, source) do
    rule
    |> requested_box(source)
    |> apply_zoom(rule)
    |> ok()
  ...
  defp resolve_base_dimensions(%__MODULE__{mode: :force} = rule, source) do
    rule
    |> requested_box(source)
    |> apply_zoom(rule)
    |> ok()
