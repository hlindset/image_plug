defmodule ImagePlug.Origin.StreamStatus do
  ...
  The holder is request-scoped and linked to the process that called
  `ImagePlug.Origin.fetch/2`. `ImagePlug.Origin.close/1` cancels the stream worker
  but intentionally leaves this holder alive so callers can still read the last
  recorded status while the response remains in scope.
  """
