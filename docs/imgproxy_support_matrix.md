...
- **Combination constellations**: a focused set crossing option intersections the
  ...
  cover+min-dims, padding+dpr). Two surfaced divergences are **quarantined** as `:equal`
  behind `--include imgproxy_triage` (excluded from the default lane) with a tracking
  issue rather than a `:diverges` floor — they *should* match once fixed: the stage-6
