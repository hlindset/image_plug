...
- **Combination constellations**: a focused set crossing option intersections the
  ...
  EXIF orientations across fit/fill/extend/rotate combinations. Three surfaced divergences
  are **quarantined** as `:equal` behind `--include imgproxy_triage` (excluded from the
  default lane) with a tracking issue rather than a `:diverges` floor — they *should*
  match once fixed: the stage-6 fit+dpr rounding fold (#199, via `extend_ar_dpr_marker`),
  the stage-10 extend east/south offset sign + clamp (#200, via `extend_offset_east_marker`),
  and the stage-7 EXIF transpose/transverse ∘ user-`rot:90` 1px edge seam (#211, via
  `exif_5_cover_rot90` and `exif_7_cover_rot90`).
