...
- **Combination constellations**: a focused set crossing option intersections the
  ...
  resampling seam, not a placement error). One surfaced divergence remains **quarantined**
  as `:equal` behind `--include imgproxy_triage` (excluded from the default lane) with a
  tracking issue rather than a `:diverges` floor — it *should* match once fixed: the
  stage-7 EXIF transpose/transverse ∘ user-`rot:90` 1px edge seam (#211, via
