...
| 5 | `crop` | `lib/image_pipe/transform/operation/crop.ex` | ✅ | Pre-resize crop with anchor / focal-point / smart / object gravity. **Anchored placement parity** (`processing/calc_position.go:37-54`): the offset's sign depends on the anchor edge — added from a near edge (west/north/center), subtracted from a far edge (east/south) — so a positive offset always moves the window inward from the named edge; offsets are rounded to an even integer before composing with the integer origin (`ScaleToEven`/`RoundToEven`, `imath/imath.go`). Center placement uses `ShrinkToEven(bounds - crop + 1, 2)` (`processing/calc_position.go:37-38`). Focus-point gravity places the window from the focus coords alone, with no offset term (`processing/calc_position.go:16-21`); ImagePipe's separate crop offset is applied as a plain inward displacement consistent with the near-edge convention. Face-assist `{:smart, :face_assist}` blends attention saliency with detected faces at weight 0.7 — ImagePipe's approximation of imgproxy `smart_crop_face_detection`, whose internal combination is unspecified. |
| 6 | `scale` | `lib/image_pipe/transform/operation/resize.ex` | ✅ | `fit`/`fill`/`fill-down`/`force`/`auto`, enlarge, min-width/height, zoom, dpr. Pro `resizing_algorithm` (`ra`) missing. **Quarter-turn orientation parity:** imgproxy's `calcScale` resolves scale in the **display** frame — the source dims are already swapped by `ExtractGeometry` (`processing/prepare.go:11-22,270`) — and the `min-width`/`min-height` floor couples the two axes cross-wise there (`processing/prepare.go:146-158`); `scale` (`processing/scale.go:10-12`) then swaps only the final scale factors to apply them to the stored pixels. ImagePipe reproduces this for a pending quarter turn by resolving against display-frame source dims and swapping the resolved result dims back into the storage frame (`PlanExecutor.cover_resize_and_crop_display_frame`), rather than swapping the request and resolving against storage — the latter is the algebraic dual only when there is no cross-axis min-dim coupling, so it holds for plain fit/fill but breaks the quarter-turn cover path. |
| 7 | `rotateAndFlip` | `.../transform/pending_orientation.ex`, `.../transform/orientation_flush.ex`, `.../transform/orientation.ex`, `.../operation/rotate.ex`, `.../operation/flip.ex` | ✅ | EXIF auto-orient + user rotate/flip are carried as deferred `pending_orientation` state and applied **late** at the orientation-flush boundary — **after** crop/resize, with crop gravity and resize dimensions compensated into the storage frame (`orientation.ex`, a port of imgproxy `gravity.go` `RotateAndFlip`) so the observable result matches. Compose suborder EXIF → user-rotate → user-flip; EXIF auto-orient is the default. Flush streams EXIF orientations 1/2 and materializes 3–8 (and any quarter/half-turn user rotate or vertical flip). The same storage↔display compensation extends to shrink-on-load: a gravity crop carrying a pending quarter turn swaps the storage-frame `decode_shrink` per-axis factors before rescaling its display-frame dims (#185). imgproxy rescales crop dims in the **display** frame (its `SrcWidth`/preshrink are `ExtractGeometry`-swapped — `prepare.go`); ImagePipe reaches the same result by swapping the storage-frame factors ahead of the quarter-turn crop swap. The `orientation.ex` port covers the rotation/flip type bijection (`processing/gravity.go:8-57`) and the per-angle offset switches inside `RotateAndFlip` (`processing/gravity.go:88-156`), each keyed on the already-remapped gravity type. A focus-point's coords transform by the focus-fraction rule (`1 - x`, axis-swap); ImagePipe's *separate* crop offset — which imgproxy's focus path has no analog for (`processing/calc_position.go` uses only the focus coords) — is a plain displacement and transforms like the `GravityCenter` vector (negate/axis-swap), **not** the fraction rule (applying the fraction rule injected a spurious 1px shift at 90/270). A centered crop with an odd extent difference rounds the discarded pixel toward the near (top/left) display edge via `ShrinkToEven` (`processing/calc_position.go:37-38`); because ImagePipe crops in the storage frame and flushes orientation afterward, it flips that discard side on each storage axis whose direction the flush reverses, so the kept pixel still lands on imgproxy's display edge (#146 Bug 2). Smart/object gravity is never remapped — it materializes and the auto-flush runs it on already-oriented pixels. The EXIF tag → (angle, horizontal-mirror) seed matches imgproxy `angleFlip` (`processing/prepare.go`): 3/4→180°, 5/6→90°, 7/8→270°, horizontal mirror on 2/4/5/7. |
| 8 | `cropToResult` | `lib/image_pipe/transform/operation/crop.ex` (result crop after a fill-style resize) | ✅ | `Resize` deliberately does **not** crop: for `fill`/`fill_down` it resizes to cover dimensions, then plan execution emits a **separate** crop to the result size. Under a pending quarter turn this result-crop carries the display-frame target dims and is swapped + gravity/offset-remapped into the storage frame by `PlanExecutor.compensate_crop` (the resize having been resolved in the display frame — see the `scale` row), so cropping pre-flush then flushing matches cropping the oriented image. |
