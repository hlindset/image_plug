...
- `ImagePlug.Plan.Operation.FlattenBackground`: composite current alpha over an
  opaque color.
- `ImagePlug.Plan.Color`: canonical product-neutral opaque sRGB color data.
...
- `ImagePlug.Transform.Operation.FlattenBackground`: resolved alpha flattening.
...
transparent pixels generated by earlier canvas or padding operations, and it
does not change dimensions.

`ImagePlug.Plan.Color` is the canonical Plan color model. The first slice
supports opaque sRGB RGB channels and serializes into structured cache key data.
...
| `pd:10/bg:f00` | `Padding`, `FlattenBackground` |
