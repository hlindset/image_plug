...
- **Held (perf-only, deferred to Plan B):** the `:access`-preserving direct `new_from_buffer` adapter and its libvips-boundary test, and the `[`-filename buffer-load — these need the Plan B decode adapter to be testable honestly and are not correctness/security issues. Added behavioral notes 6 (unsupported-format timing) and 7 (filename association) for the remaining low-severity observations.
