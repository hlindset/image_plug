...
**Architecture:** `ImagePipe.Source.Response` gains an optional `path` field alongside `stream` (additive, so HTTP/S3/test adapters keep compiling unchanged). The file adapter returns a validated `path`; HTTP/S3 keep returning a guarded `stream`. `Request.Processor` turns either into a seekable libvips input — `Image.open(path)` for paths, or **drain-the-guarded-stream-to-binary then `Image.from_binary(binary)`** for streams — with the drain failing closed (body-limit / stream errors surface as `{:source, …}` *before* decode). Output pixels are identical to today, so all pixel-exact tests still pass. (`from_binary/2` — not `Image.open(binary)` — so libvips detects the format from bytes for any supported format, matching the old stream loader; see Behavioral notes for the `:access` and file-limit deltas.)
...
3. **Drained buffers decode via `Image.from_binary/2`, not `Image.open(binary)`.** `Image.open/2` on a binary only signature-matches a subset of formats (JPEG/PNG/WebP/GIF/TIFF/HEIF-AVIF/SVG) and would misroute any *other* libvips-supported format — notably JPEG 2000 (`jp2kload`) and JPEG-XL (`jxlload`), both in our `SourceFormat` set — as a filesystem path (`:enoent`). The old streaming loader (`new_from_enum`) detected the format from bytes for any supported format; `from_binary/2` (→ `new_from_buffer`) restores that parity. (Not triggerable in the dev build's minimal libvips, which lacks those loaders, hence no fixture-based test — see [[project_image_vision_inference_env_limitation]] for the precedent on env-limited coverage.)
4. **`:access` is NOT preserved at the libvips loader for the drained-buffer path.** `Image.from_binary/2` deletes `:access` before calling `new_from_buffer` (deps/image `from_binary/2`), so the HTTP/S3 buffer decode runs at libvips' default (random) access regardless of the planner's `:sequential` selection. This is **perf-only** — access mode does not change output pixels, and `fail_on:` *is* preserved. The planner's sequential selection on the buffer path is restored in **Plan B** via a direct `new_from_buffer` adapter (which Plan B needs anyway to pass `shrink:`/`scale:`). The file-path branch keeps `:access` (it flows through `new_from_file`).
...
- Wire-level plug tests asserting `access:`/`fail_on:` decode options (e.g. `"safe one-pass resize opens origin with sequential access"`, `plug_test.exs:1839+`): these still pass — they assert the options the Processor *computes and hands to the injected open module*, which is unchanged. Note this is the Processor boundary, **not** the libvips loader: for the drained-buffer path `from_binary/2` drops `:access` (see Behavioral note 4), so libvips itself decodes at default access. The tests are correct as decode-option-computation contracts; **do not rewrite them**, but do not read them as proof that libvips received `:access`.
...
**Type consistency:** `%Response{stream: ..., path: ...}` used identically across Tasks 1–4; `seekable_input/1` returns `{:ok, {:path, path} | {:buffer, binary}} | {:error, {:source, _}}` consumed by the `with` in `decode_source_response/3`, which dispatches via `open_seekable_input/3` (paths → `Image.open/2`, buffers → `Image.from_binary/2`, injected module → `open/2` on the raw value); `wrap_response/2` clauses (Task 2) match the struct from Task 1; error tags (`:body_too_large`, `:stream_exception`, `:invalid_adapter_result`, `{:input_limit, …}`) are preserved, not renamed.
...
2. Processor: open the seekable input lazily, read header dims + format, validate `max_input_pixels` on **original** dims, then re-open with the load option when `load_shrink > 1`.
   - **Decode adapter (replaces Plan A's `Image.open`/`from_binary` defaults):** add an ImagePipe-owned open seam that calls `Vix.Vips.Image.new_from_file/2` (path) and `Vix.Vips.Image.new_from_buffer/2` (buffer) **directly**, so loader options survive — this restores `:access` on the buffer path (Plan A's `from_binary/2` drops it; see Behavioral note 4) **and** carries `shrink:`/`scale:`. Tag the open module's contract as `open_path/2` + `open_buffer/2` (or tagged input) and update the test stubs accordingly. Add a libvips-boundary test proving `:access` and `fail_on:` (and, when set, `shrink:`/`scale:`) reach the Vix loader — Plan A's wire tests only pin the Processor boundary.
   - **File paths containing `[`:** buffer-load bracketed filenames (read bytes → `new_from_buffer`) so they are not split as loader-option syntax (Plan A known limitation).
...
A later external review caught a real **fidelity regression** in the as-shipped Plan A: drained buffers were decoded with `Image.open(binary)`, whose binary-signature whitelist omits JPEG 2000 / JPEG-XL (both in `SourceFormat`), so those would misroute as filesystem paths — whereas the old stream loader detected any libvips format. Fixed by routing buffers through `Image.from_binary/2` (commit on this branch) + a multi-chunk drain test. The same review correctly flagged that `from_binary/2` drops `:access` (behavioral note 4, perf-only, restored in Plan B's decode adapter), that file sources lose `max_body_bytes` enforcement (note 5), the `[`-filename split (known limitation), and that "byte-identical" was overstated (downgraded to pixel-identical + existing wire tests). **Not adopted:** a stricter `wrap_response`/`seekable_input` guard + test for a both-`path`-and-`stream` Response — no in-repo adapter can construct that shape, and the repo's guidelines explicitly forbid guarding/testing impossible internal misuse; the existing path-precedence clause is the intended behavior.
