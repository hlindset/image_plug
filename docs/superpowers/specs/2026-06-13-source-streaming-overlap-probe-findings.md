...
Harness: `bench/source_overlap_probe.exs` — **not on `main`.** It requires the
experimental vix fork (`new_from_enum(mode:)` / `SourceSpool`), which the spike does not
land on `main`, so the harness + the vix bump live on branch
[`spike/source-streaming-overlap-probe`](https://github.com/hlindset/image_pipe/tree/spike/source-streaming-overlap-probe)
([PR #276](https://github.com/hlindset/image_pipe/pull/276), intentionally unmerged).
This doc is the durable record; check out that branch to run the harness.
...
The harness and the vix fork it needs are **not on `main`** — check out
`spike/source-streaming-overlap-probe` ([PR #276](https://github.com/hlindset/image_pipe/pull/276)) first, then:
