...
**Spawns follow-ups:** [#267](https://github.com/hlindset/image_pipe/issues/267) — customizable transform/processing error → HTTP status mapping; [#268](https://github.com/hlindset/image_pipe/issues/268) → **landed as [#269](https://github.com/hlindset/image_pipe/pull/269)** (flatten-to-background for explicit non-alpha output formats). #269 deferred two tests to #253 — the genuine B_W+alpha→flatten case (needs the IIIF `gray` producer) and an ETag-discrimination test (needs a non-white `flatten_background` producer).
...
- **`gray` op:** sequential-vs-random equivalence + property test (the AGENTS.md gate); **§S1** split by alpha support: (a) on an **RGBA** source, `gray.png`/`gray.webp`/`gray.avif` round-trip `200` with a non-opaque alpha band intact (gray preserves alpha); (b) on an **opaque** source, `gray.jpg` → `200` valid JPEG with RGB bands equal at sampled points (true desaturation); (c) on an **RGBA** source, `gray.jpg` → `200` valid (opaque) JPEG — the genuine B_W+alpha→flatten path that **#269** deferred to this producer. (The ETag-discrimination test #269 also deferred stays deferred: no producer sets a non-white `flatten_background` yet, even in IIIF.)
