...
- Cache only successful encoded responses, with deterministic keys; cache errors fail open by default unless `fail_on_cache_error: true`. Which fields compose the key is owned by `ImagePipe.Cache.Key` and its tests — read those rather than maintaining a field list here.
